...
const EditableCell = ({ value, onChange, className = '', isEditable = true }) => {
  ...
  if (editing) {
    return (
      <input
        ...
        className={`w-full h-full px-2 py-1 text-right text-sm border border-zinc-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-zinc-900/10 focus:border-zinc-400 shadow-sm [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none ${className}`}
      />
    );
  }
  ...
  return (
    <div
      ...
      className={`w-full h-full px-2 py-1 text-right text-sm rounded-md transition-colors select-none ${isEditable ? 'cursor-pointer hover:bg-zinc-100' : ''} ${className}`}
...
const ProfitLossSearchEducation = ({ companyPeriods, onSave, isEditMode, view = 'table' }) => {
  ...
  const trendData = useMemo(() => {
    ...
    return recent.map(p => {
      ...
      const inc = iRows.reduce((sum, row) => sum + (p.income?.[row.key] || 0), 0);
  ...
  return (
    <div className="space-y-4">
          ...
          <table className="w-full text-sm border-collapse min-w-[500px]">
            ...
            <tbody className="text-zinc-700">
              ...
              <tr className="bg-amber-50/30 group">
                <td className="px-4 py-2 font-bold text-amber-700 text-[11px] uppercase tracking-widest border-b border-zinc-100 mt-2">
                  <div className="flex items-center justify-between">
                    <span>Add Back</span>
                    {isEditMode && (
                      <button
                        onClick={() => setShowAddBackModal(true)}
                        className="px-2 py-0.5 text-[10px] font-semibold text-amber-600 hover:bg-white/50 rounded transition-colors opacity-0 group-hover:opacity-100"
                      >
                        + Add Item
                      </button>
                    )}
              ...
              {(currentPeriod.addBacks || []).map(ab => (
                <tr key={ab.id} className="hover:bg-amber-50/10 transition-colors group">
                  ...
                  <td className="px-1 py-1 border-b border-zinc-50 pr-6 text-notion-black font-semibold">
                    <EditableCell isEditable={isEditMode} value={ab.value || 0} onChange={v => handleUpdateAddBack(ab.id, 'value', v)} className="py-1" />
                  </td>
                  <td className="border-b border-zinc-50"></td>
                </tr>
              ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-2">
        {[
          { label: 'Total Income', value: totalIncome, color: 'text-zinc-900', bg: 'bg-white' },
          { label: 'Total Expenses', value: totalExpenses, color: 'text-zinc-900', bg: 'bg-white' },
          { label: 'Net P&L', value: netProfitLoss, color: netProfitLoss >= 0 ? 'text-emerald-600' : 'text-red-600', bg: 'bg-white' },
          { label: 'After Adjustment', value: netProfitLossAfterAdjustment, color: netProfitLossAfterAdjustment >= 0 ? 'text-emerald-600' : 'text-red-600', bg: 'bg-zinc-50' },
        ].map(card => (
          <div key={card.label} className={`notion-card p-5 ${card.bg}`}>
            <p className="text-[11px] font-medium text-zinc-500 uppercase tracking-wider mb-2">{card.label}</p>
            <p className={`text-2xl font-semibold tracking-tight ${card.color}`}>{fmt(card.value)}</p>
          </div>
        ))}
      </div>
        </>
            ...
            <table className="w-full text-sm border-collapse min-w-[800px]">
              <thead>
                <tr className="border-b border-zinc-200 bg-zinc-50/50">
                  <th className="text-left px-4 py-3 font-medium text-zinc-500 text-[11px] uppercase tracking-wider sticky left-0 z-10 bg-zinc-50/90 min-w-[200px]">
                    Category
                  </th>
                  {trendData.map(p => (
                    <th key={p.name} className="text-right px-4 py-3 font-medium text-zinc-500 text-[11px] uppercase tracking-wider min-w-[100px]">
                      {p.name}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="text-zinc-700">
                <tr className="bg-blue-50/30">
                  <td className="px-4 py-2 font-bold text-blue-500 text-[11px] uppercase tracking-widest border-b border-zinc-100 sticky left-0 z-10 bg-blue-50">Income</td>
                ...
                <tr className="bg-rose-50/30 mt-2">
                  <td className="px-4 py-2 font-bold text-red-500 text-[11px] uppercase tracking-widest border-b border-zinc-100 sticky left-0 z-10 bg-rose-50 mt-2">Expenses</td>
                  {trendData.map(p => <td key={p.name} className="border-b border-zinc-100"></td>)}
                </tr>
                {(currentPeriod.expenseRows || []).map(row => (
                ...
                <tr className="bg-emerald-50/60 font-bold text-sm">
                  <td className="px-4 py-3 text-emerald-700 border-b border-zinc-200 sticky left-0 z-10 bg-emerald-50">Net Profit & Loss</td>
                  {trendData.map(p => (
