...
// A "custom row" is a user-added row with a label and 12 month values
const defaultCustomRow = () => ({
...
const ReadCell = ({ value, highlight }) => {
  ...
  return (
    <td className={`border border-zinc-200 px-2 py-1.5 text-right text-xs font-medium whitespace-nowrap ${isNeg ? 'text-red-600' : 'text-zinc-800'} ${highlight ? 'bg-blue-50' : ''}`}>
...
const EditCell = ({ value, onChange, highlight }) => {
  ...
  return (
    <td
      className={`border border-zinc-200 px-0 py-0 text-right text-xs cursor-text ${highlight ? 'bg-blue-50 hover:bg-blue-100' : 'bg-white hover:bg-zinc-50'}`}
    ...
    >
      {editing ? (
        <input
          ...
          onBlur={() => { setEditing(false); onChange(raw); }}
          onKeyDown={e => { if (e.key === 'Enter' || e.key === 'Tab') { setEditing(false); onChange(raw); } }}
          className={`w-full px-2 py-1.5 text-right text-xs outline-none border-2 border-notion-blue ${highlight ? 'bg-blue-100' : 'bg-yellow-50'}`}
        />
      ) : (
...
const StaffCard = ({ staff, year, onUpdate, onDelete, editing }) => {
  ...
  const updateCustomRow = (rowId, idx, value) => {
    const rows = staff.customRows.map(r =>
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr className="bg-zinc-100">
              ...
              {MONTHS.map(m => (
                <th key={m} className="border border-zinc-200 px-2 py-2 text-center font-bold text-zinc-600 min-w-[72px]">{m}-{String(year).slice(2)}</th>
              ))}
          ...
          <tbody>
            ...
            <tr>
              ...
              {staff.basicSalary.map((v, i) => <Cell key={i} value={v} onChange={val => updateMonth('basicSalary', i, val)} editing={editing} />)}
            ...
            <tr>
              ...
              {staff.superannuation.map((v, i) => <Cell key={i} value={v} onChange={val => updateMonth('superannuation', i, val)} editing={editing} />)}
              <td className="border border-zinc-200 px-2 py-1.5 text-right text-xs font-semibold bg-zinc-50">{totalSuper ? fmt(totalSuper) : ''}</td>
              {editing && <td className="border border-zinc-200" />}
            </tr>

            {/* Total Cost (calculated, never editable) */}
            ...
            {/* Spacer */}
            <tr><td colSpan={editing ? 15 : 14} className="py-0.5 bg-white border-x-0 border-zinc-100" /></tr>

            {/* Revenue Earned */}
            <tr>
              <td className="border border-zinc-200 px-3 py-1.5 text-xs font-semibold text-blue-700 bg-blue-50">Revenue Earned</td>
              {staff.revenueEarned.map((v, i) => <Cell key={i} value={v} onChange={val => updateMonth('revenueEarned', i, val)} highlight editing={editing} />)}
            ...
            <tr>
              ...
              {staff.serviceFeeRef.map((v, i) => <Cell key={i} value={v} onChange={val => updateMonth('serviceFeeRef', i, val)} highlight editing={editing} />)}
              <td className="border border-zinc-200 px-2 py-1.5 bg-blue-50" />
              {editing && <td className="border border-zinc-200 bg-blue-50" />}
            </tr>

            {/* Custom Rows */}
            {(staff.customRows || []).map(row => (
              <tr key={row.id} className={row.highlight ? 'bg-blue-50' : ''}>
                <td className={`border border-zinc-200 px-3 py-1.5 text-xs ${row.highlight ? 'text-blue-700 font-semibold' : 'text-zinc-600'}`}>
                  {editing ? (
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        className="flex-1 bg-transparent outline-none border-b border-zinc-300 focus:border-notion-blue text-xs"
                        placeholder="Row label"
                      />
                      <button onClick={() => toggleRowHighlight(row.id)} title="Toggle blue highlight" className={`p-0.5 rounded ${row.highlight ? 'text-blue-500' : 'text-zinc-400'} hover:text-blue-600`}>
                        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><circle cx="10" cy="10" r="8"/></svg>
                      </button>
                    </div>
                  ) : row.label}
                </td>
                {row.values.map((v, i) => <Cell key={i} value={v} onChange={val => updateCustomRow(row.id, i, val)} highlight={row.highlight} editing={editing} />)}
                <td className={`border border-zinc-200 px-2 py-1.5 text-right text-xs font-semibold ${row.highlight ? 'bg-blue-50 text-blue-700' : 'bg-zinc-50'}`}>
                  {rowTotal(row.values) ? fmt(rowTotal(row.values)) : ''}
                </td>
                {editing && (
                  <td className="border border-zinc-200 text-center">
                    <button onClick={() => removeCustomRow(row.id)} className="p-1 text-zinc-300 hover:text-red-500 transition-colors" title="Remove row">
            ...
            {editing && (
              <tr>
                <td colSpan={editing ? 15 : 14} className="border border-zinc-200 px-3 py-1.5">
                  <button onClick={addCustomRow} className="flex items-center gap-1 text-xs text-notion-blue hover:text-blue-700 font-semibold transition-colors">
    ...
    <div className="p-6 max-w-full">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-colors shadow-sm ${editing ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-zinc-100 text-zinc-700 hover:bg-zinc-200'}`}
          >
            {editing ? (
              <>
          ...
          {/* Add Staff (only while editing) */}
          {editing && (
            <button onClick={addStaff} className="flex items-center gap-2 px-4 py-2 bg-notion-blue text-white rounded-lg text-sm font-bold shadow-sm hover:bg-blue-700 transition-colors">
      ...
      {editing && (
        <div className="flex items-center gap-2 mb-4 px-4 py-2.5 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800 font-medium">
          ...
          Edit mode is active. Click any cell to enter a value. Use "Add Row" inside each staff card to insert custom rows. Click <strong>&nbsp;Done Editing&nbsp;</strong> when finished.
        </div>
      )}
      ...
      {staffList.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <button onClick={() => { setEditing(true); addStaff(); }} className="px-4 py-2 bg-notion-blue text-white rounded-lg text-sm font-bold hover:bg-blue-700 transition-colors">
